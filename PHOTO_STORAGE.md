...
从 v3.28.0 起，双轨/共同记忆固定照片、动态故事模块照片、自由照片框和 28 张拍立得也不再把新图片压成小体积 Base64。浏览器生成最长边 1800px、质量 0.88 的展示 JPEG 后写入 `assets/`，内容快照只保存公开媒体地址。全站文字和各模块状态由 `/api/site-state` 统一保存；另一台设备按 `updatedAt` 应用更新。本机 localStorage 仍保留副本，用于网络中断时继续展示。
