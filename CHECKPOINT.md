...
> **最后更新**：2026-07-18
> **当前版本**：v3.30.6（`index.html` 内 `APP_VERSION` 已同步为 "3.30.6"）
...
| **首页编辑/展示同位** | 恋爱天数设置面板改为独立浮层，同时固定保留原编辑预览的垂直空间；编辑前、保存后标题、数字、按钮及自由文本框保持第一张截图中的坐标关系 | `index.html` |
| **首页按钮位置锁定** | 首次进入编辑时记录“翻开我们的故事”的视口坐标，并以等尺寸占位保留原排版；调整文字或恋爱天数字号、保存及退出编辑均不会推动按钮 | `index.html` |
| **文本框直接删除** | 原有固定文本框点击删除后立即完全隐藏，不再保留半透明占位；误删可用“撤销”，需要整体恢复时可用“恢复文字” | `index.html` |
...
| **主叙事照片直传** | 双轨 2 张与共同记忆 3 张照片位在编辑模式可直接上传/更换；新照片上传为 1800px 高质量 R2 资产，内容状态只保存云端地址 | `index.html` / `cloudflare-r2-worker/` |
| **动态照片模块直传** | 工具栏“加照片”创建的动态模块可独立上传/更换真实照片；新照片上传为高质量 R2 资产并随站点快照跨端显示 | `index.html` / `cloudflare-r2-worker/` |
